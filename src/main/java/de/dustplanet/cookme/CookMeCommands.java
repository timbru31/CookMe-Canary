...
public class CookMeCommands implements CommandListener {
    ...
    @Command(aliases = { "" },
	    description = "CookMe admin control",
	    permissions = { "cookme.admin" },
	    toolTip = "/cookme help")
    public void onCommand (MessageReceiver caller, String[] args) {
	// reload
	if (caller.hasPermission("cookme.admin") || !plugin.permissions) {
		...
		if (caller instanceof Player) {
		    ...
		    player.setHunger(10);
		    player.sendMessage(TextFormat.LIGHT_GREEN + "Food level reduced!");
		}
		else {
		    caller.message(TextFormat.RED + "Debug mode can only be used ingame!");
		}
	    }
		...
		if (args[1].equalsIgnoreCase("cooldown")) {
			...
			// Cooldown not a number?
			    ...
			    plugin.message(caller, message, null, null);
			    return;
			}
			plugin.config.setInt("configuration.cooldown", cooldown);
			plugin.config.save();
			String message = plugin.localization.getString("changed_cooldown");
			plugin.message(caller, message, Integer.toString(cooldown), null);
			plugin.cooldownManager.setCooldown(cooldown);
		    } else {
			String message = plugin.localization.getString("wrong_arguments");
			plugin.message(caller, message, null, null);
		    }
		}
		// Duration
			...
			if (args.length > 4) {
			    ...
			    plugin.message(caller, message, Integer.toString(duration), null);
			} else {
			    String message = plugin.localization.getString("wrong_arguments");
			    plugin.message(caller, message, null, null);
			}
		...
		// Effect
			...
			plugin.message(caller, message, effect, Double.toString(percentage));
		    } else {
			String message = plugin.localization.getString("wrong_arguments");
			plugin.message(caller, message, null, null);
		    }
		...
		if (args.length > 2 && args[2].equalsIgnoreCase("permissions")) {
		    cookMeEnablePermissions(caller);
		}
		// messages
		if (args.length > 2 && args[2].equalsIgnoreCase("messages")) {
		    cookMeEnableMessages(caller);
		} else {
		    String message = plugin.localization.getString("wrong_arguments");
		    plugin.message(caller, message, null, null);
		}
		...
		// permissions
		if (args.length > 2 && args[2].equalsIgnoreCase("permissions")) {
		    cookMeDisablePermissions(caller);
		}
		// messages
		if (args.length > 2 && args[2].equalsIgnoreCase("messages")) {
		    cookMeDisableMessages(caller);
		} else {
		    String message = plugin.localization.getString("wrong_arguments");
		    plugin.message(caller, message, null, null);
		}
    ...
    private void cookMeHelp(MessageReceiver receiver) {
	for (int i = 1; i <= 11; i++) {
	    String message = plugin.localization.getString("help_" + Integer.toString(i));
	    plugin.message(receiver, message, null, null);
	}
    }

    // Reloads the config with /cookme reload
    private void cookMeReload(MessageReceiver receiver) {
    ...
    private void cookMeEnablePermissions(MessageReceiver receiver) {
    ...
    private void cookMeDisablePermissions(MessageReceiver receiver) {
    ...
    private void cookMeEnableMessages(MessageReceiver receiver) {
    ...
    private void cookMeDisableMessages(MessageReceiver receiver) {
